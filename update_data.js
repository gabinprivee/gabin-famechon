...
const dataTsContent = `import { Category, Level, Achievement } from './types';
...
const generateLevels = (categoryId: string, themes: string[], bossTask: string, codeSnippets: Record<number, string> = {}): Level[] => {
  ...
  for (let i = 1; i <= 100; i++) {
    ...
    if (levelInTheme === 0) {
      ...
      description = \`Avant de plonger dans le code complexe, comprenons la théorie.\\n\\nDans un jeu vidéo, la thématique "\${theme}" nécessite toujours d'initialiser un état. On utilise souvent des variables pour garder la trace des positions, scores ou autres états.\\n\\nExemple :\\nlet positionX = 0;\\nconst gravite = 9.81;\`;
      task = \`Tutoriel : Déclare au moins deux variables pour stocker des états liés à ce thème (ex: vitesse, munitions, score) en utilisant 'let' ou 'const'.\`;
      testCode = \`if (!code.includes('let ') && !code.includes('const ')) throw new Error('Tutoriel non validé : Déclare tes variables (let, const).');\`;
      codeSnippet = \`// ----------------------------------------\\n// THÉORIE 1 : État pour \${theme}\\n// ----------------------------------------\\n// Déclare tes variables ici :\\n\\n\`;
    } else if (levelInTheme === 1) {
      title = \`Niveau \${i} : [THÉORIE 2/2] - Logique de '\${theme}'\`;
      description = \`Deuxième étape de théorie. Comment manipuler ces données ?\\n\\nOn utilise des structures de contrôle (if, for, while) pour mettre à jour la logique du jeu.\\n\\nExemple :\\nif (estAuSol) { vitesseY = 0; }\\n\\nCela permet au jeu de réagir en temps réel.\`;
      task = \`Tutoriel (Fin) : Écris une condition (if/else) ou une boucle (for/while) pour modifier fictivement tes variables d'état.\`;
      testCode = \`if (!code.match(/if\\s*\\(/) && !code.match(/for\\s*\\(/) && !code.match(/while\\s*\\(/)) throw new Error('Tutoriel non validé : Utilise au moins une condition (if) ou une boucle (for/while).');\`;
      codeSnippet = \`// ----------------------------------------\\n// THÉORIE 2 : Logique pour \${theme}\\n// ----------------------------------------\\n// Ajoute tes conditions/boucles ici :\\n\\n\`;
    } else {
      const challenges = [
        ...
        { title: "Mouvement & Maths", desc: "Nous avons besoin de dynamisme. Applique des transformations ou de l'arithmétique.", task: "Modifie les valeurs avec des opérateurs mathématiques (+, -, *, /) pour créer de l'action.", test: "if (!code.match(/[+\\-*\\/]=/) && !code.match(/[\\w]+[\\+\\-*\\/][\\w\\d]+/)) throw new Error('Utilise des opérateurs mathématiques (+=, -=, +, -, *, /) pour simuler le comportement.');" },
        ...
        { title: "Refactoring", desc: "Le code commence à être lourd. Rend-le plus propre.", task: "Réfractore ton code. Encapsule ta logique répétitive dans une fonction dédiée.", test: "if (!code.match(/function\\s+[a-zA-Z_]+\\s*\\(/) && !code.match(/[a-zA-Z_]+\\s*=\\s*\\(/)) throw new Error('Crée au moins une fonction nommée pour encapsuler ton code.');" },
    ...
    if (i === 1) {
      ...
      task = \`Crée une boucle de jeu basique en utilisant requestAnimationFrame en JavaScript.\`;
